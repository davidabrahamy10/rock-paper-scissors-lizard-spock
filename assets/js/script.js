...
/** Loops through all buttons and removes the color
 * except for the button thats been selected by the player
 */
...
/** Takes the player choice and matches it to corresponding image and alt
 * selects a random number as the computerer selection and mathes it to a image
 */
...
/** Checks to see who the winner is by running through all the
 * possible outcomes and returns a string
 */
...
/** Takes the outcome from checkWinner function and updates scores accordingly */
...
/** Resets the score when clicked on */
function resetScore() {
    playerScore = 0;
    computerScore = 0;
    drawScore = 0;
...
/** The player that reaches 10 first wins the game 
 * and the function sends the message
 */
function winGame() {
    if (playerScore == 10) {
